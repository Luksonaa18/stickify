  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <h1 className="font-bold italic text-white text-sm">Drag to reorder or merge</h1>
...
const StickyNotes = () => {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    const { over } = event;
    setOverNoteId(over?.id?.toString() || null);
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (overNoteId === over.id && overNoteId !== active.id) {
      // Merge notes
