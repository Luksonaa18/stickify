...
const SortableItem = ({
  ...
  return (
    <motion.div
      ...
      {...attributes}
      {...listeners}
      className={`relative rounded-lg p-3 shadow-md select-none transition-all duration-200 ${
        isDragging ? "opacity-50 scale-95" : ""
      }`}
    >
      <div className="flex items-center justify-between mb-2">
        <h1 className="font-bold italic text-white text-sm">
          Drag to reorder or merge (hold Shift)
        </h1>
...
const StickyNotes = () => {
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    const shouldMerge =
      isShiftDown &&
      activeNote.textArea.trim() &&
      overNote.textArea.trim();

    if (shouldMerge) {
      setNotes((prev) => {
        const updated = [...prev];
        const activeIndex = updated.findIndex((n) => n.id === active.id);
        const overIndex = updated.findIndex((n) => n.id === over.id);
        if (activeIndex === -1 || overIndex === -1) return prev;

        updated[overIndex] = {
          ...updated[overIndex],
          textArea: `${updated[overIndex].textArea}\n\n${updated[activeIndex].textArea}`.trim(),
          isEditing: true,
        };
